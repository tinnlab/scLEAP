...
from sclmct.loss import CellTextContrastiveLoss, TextCentersContrastiveLoss
	...
	def __init__(self, input_dim, hidden_dim, output_dim, n_layers=3, block_type='v2', use_linear_out = False):
		...
		for i in range(n_layers):
			...
			if block_type == 'v1':
				...
				self.residual_blocks.append(ResidualBlockV2(in_dim, out_dim))
	def forward(self, x):
		"""
...
class TrainWrapperCLIPStyle(pl.LightningModule):
	def __init__(self,
				 ...
				 arc_m: float = 0.5,
				 f_gamma: float = 2.0,
				 loss_type: str = 'combine',
				 l_lambda: float = 0.7,
				 clip_loss_type: str = 'label'
				 ):
		super().__init__()
		...
		self.encoder = CellEncoder(input_dim, hidden_dim, self.output_dim, n_layers=n_cell_layers, block_type=block_type, use_linear_out=False)
		...
		self.clip_loss = CellTextContrastiveLoss(s=arc_s, m=arc_m, focal_gamma=f_gamma)
		
		self.text_center_loss = TextCentersContrastiveLoss(init_temperature=0.07, min_temp=1.0, max_temp=100.0)
	...
	def get_class_text_centers(self, labels=None, only_centers=False):
		...
		for class_id in sorted(labels):
			...
			if self.training and hidden_states_10.size(0) > self.n_prompts:
				...
				idx = torch.randperm(hidden_states_10.size(0), generator=g)[:self.n_prompts]
		...
		if only_centers:
			...
			return torch.tensor(class_ids, device=self.device), F.normalize(torch.stack(centers), dim=-1)
		
		else:
			label_tensor = torch.tensor(all_labels, device=self.device)
			embedding_tensor = torch.cat(all_embeddings, dim=0)
			return label_tensor, F.normalize(embedding_tensor, dim=-1)
	...
	def training_step(self, batch, batch_idx):
		...
		loss = self.l_lambda * loss_clip + (1-self.l_lambda) * loss_text_text
		
		## logging
		self.log("train_loss", loss, on_step=False, on_epoch=True, prog_bar=True)
		self.log("loss_clip", loss_clip, on_step=False, on_epoch=True, prog_bar=True)
...
def process_new_celltypes(model, cell_types, label_prompts = None):
    ...
    model.text_encoder_wrapper.cache_path = "/data/dungp/projects/single-Cell/Sy-Code/src_LLM_CLIP/cache/temp_cache.pt"
